%GENERATE_DOTS_CONTROL   Control dot patterns.
%   GENERATE_DOTS_CONTROL generates dot patterns with summed area and 
...
for number = minnumber:maxnumber
    ...
    while i <= images
        ...
        % draw dots
        for j=1:number
            fill(x*radius(j) + xpos(j),y*radius(j) + ypos(j), dotcolor);
            hold on
        end    
        ...
        filename = strcat(num2str(400+100*number+i),'.bmp')
